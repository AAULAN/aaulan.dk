...
class CheckTicket {
	...
	public function fire($job,$data) {
		...
		$apiUrl = "https://api.studentersamfundet.dk/aaulan";
		
		$ch = curl_init();
        $query = http_build_query(array('ticketnumber'=>$data['ticketnumber'],'access_token'=>'gwzscbowxsjytehoglm6nmhv6dwsvfdi3i9miydvwjchkc2y8f8xpnsmle3ussc1'));
		...
		if ($cr === FALSE) {
			...
			if ($json ===  NULL) {
				Log::error('CheckTicket, json error');
			} else {
